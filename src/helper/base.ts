...
class Base {
  ...
  get(path: string, page = 1, perPage = this._perPage) {
    path += path.includes('?') ? '' : '?';
  ...
  all(path: string, perPage = this._perPage, runners = this._parallel) {
  ...
  private static createGetTasks(path: string, to: number, from = 2, perPage = 100) {
    let tasks: Task[] = [];
  ...
  private getParallel(tasks: Task[], collect: any[] = [], runners = this._parallel) {
  ...
  private parallel(tasks: Task[], worker: (task: Task, done: Function) => any, runners = this._parallel) {
